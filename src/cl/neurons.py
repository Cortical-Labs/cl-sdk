...
class Neurons:
    ...
    def _advance_elapsed_frames(self, frame_count: int = 0):
        ...
        if blocking_mode and not self._use_accelerated_time:
            ...
            Neurons._more_accurate_sleep(wait_secs)
    ...
    @staticmethod
    def _more_accurate_sleep(seconds: float):
        """
        (Mock only) Attempt a more accurate sleep by wasting CPU for a short duration.

        Args:
            seconds: number of total seconds to sleep.
        """
        # Buffer duration derived from testing on a number of different systems
        BUFFER_SECS: float = 0.1
